...
local cache=DrawRequests.new()
assert(cache:begin(3,"first","daily") == "start")
cache:complete(3,"first",{status="complete"})
assert(cache:begin(3,"first","deep") == "conflict")
for i=1,20 do
    assert(cache:begin(3,"id"..i,"daily") == "start")
    cache:complete(3,"id"..i,{status="complete"})
end
assert(cache:begin(3,"first","daily") == "expired")

local cancelled=ReadingSession.new()
cancelled:dispatch({type="near",value=true})
cancelled:dispatch({type="begin",id="pending"})
cancelled:dispatch({type="contact"})
cancelled:dispatch({type="near",value=false})
cancelled:dispatch({type="begin",id="duplicate"})
assert(cancelled:snapshot().requestId == "pending")
cancelled:dispatch({type="failure",id="pending",uncertain=true})
cancelled:dispatch({type="near",value=true})
cancelled:dispatch({type="received",id="pending",reading={verses={{textEn="late"}}}})
assert(cancelled:snapshot().phase == "paused")
cancelled:dispatch({type="resume"})
cancelled:dispatch({type="finish"})
cancelled:dispatch({type="near",value=false})
cancelled:dispatch({type="respawn"})
cancelled:dispatch({type="closed"})
assert(cancelled:snapshot().phase == "idle" and cancelled:snapshot().reading == nil)
local rejected=ReadingSession.new()
rejected:dispatch({type="near",value=true})
rejected:dispatch({type="begin",id="rejected"})
rejected:dispatch({type="contact"})
rejected:dispatch({type="failure",id="rejected",uncertain=false})
rejected:dispatch({type="failure",id="rejected",uncertain=true})
assert(rejected:snapshot().phase == "failed")
